...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False, epsilon=1.0, alpha=0.55):
    ...
    def get_maxQ(self, state):
        ...
        dd = self.Q[self.state]
        maxQ = max(dd, key=dd.get)
    ...
    def createQ(self, state):
        ...
        if not state in self.Q.keys():
            ...
            self.Q[state] = dict(zip(va, len(va)*[0.0]))
        return
    ...
    def learn(self, state, action, reward):
        ...
        self.Q[state][action] = self.Q[state][action] * (1 - self.alpha) + self.alpha * (reward + 0.0)
        return
...
def run():
    ...
    sim = Simulator(env, update_delay = 2.0, log_metrics=True, optimized=True, display=True)
